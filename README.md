...
**ROS 2 Jazzy + Gazebo Jetty + TurboVNC desktop environment with persistent workspace.**

This container provides ROS 2 Jazzy Jalisco & Gazebo Jetty in Docker, based on the MathWorks Robotics ROS Toolbox Dockerfile, carefully modified to fit my needs, and refined with guidance from an LLM.
...
```bash
docker build -t ros2-gazebo-jetty-vnc .
```
...
```bash
mkdir -p ~/ros2-gazebo-jetty-vnc_data
```
...
```bash
docker stop ros2-gazebo-jetty-vnc-container
```

Restart with all data preserved:

```bash
docker start -d ros2-gazebo-jetty-vnc-container
```

## Sources

* ROS instructions: [ROS Jazzy Installation](https://docs.ros.org/en/jazzy/Installation/Ubuntu-Install-Debs.html)
* Gazebo instructions: [Gazebo Jetty Install](https://gazebosim.org/docs/jetty/install_ubuntu/)
